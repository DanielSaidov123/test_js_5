...
export const encrypt = async (req, res) => {
  try {
    ...
    if (req.body.cipherType == "reverse") {
      const reversedStr = req.body.message
        .split("")
        .reverse()
        .join("")
        .toUpperCase();
      const mss = await q.create(req, {
        username: req.body.username,
        cipher_type: req.body.cipherType,
        encrypted_text: reversedStr,
      });
      ...
      res
        ...
        .json({
          ...
          encryptedText: messag.encrypted_text,
        });
    }
  } catch (err) {
    console.error(err);
...
export const decryptt = async (req, res) => {
  try {
    if (!req.body.username || !req.body.password) {
      return res.status(401).json({
        message: "No password or username entered.",
    ...
    const reversedStr = messag.encrypted_text
      .split("")
      .reverse()
      .join("")
      .toLowerCase();
    ...
    res
      .status(201)
      .json({ id: req.body.messageId, decryptedText: reversedStr });
  } catch (err) {
    console.error(err);
    if (err.code === 11000) {
      return res.status(409).json({
...
export const decrygetAll = async (req, res) => {
  try {
    if (!req.body.username || !req.body.password) {
      return res.status(401).json({
        message: "No password or username entered.",
    ...
    const messag = await q.getAllbyusername(req, req.body.username);
